...
export const buildForexWeeklySchedule = (
  ...
  add(open, WeekDay.Sunday, {
    start: { hour: openHour, minute: 0 },
    end: { hour: 24, minute: 0 },
  });
  add(open, WeekDay.Monday, {
    start: { hour: 0, minute: 0 },
    end: { hour: 24, minute: 0 },
  });
  add(open, WeekDay.Tuesday, {
    start: { hour: 0, minute: 0 },
    end: { hour: 24, minute: 0 },
  });
  add(open, WeekDay.Wednesday, {
    start: { hour: 0, minute: 0 },
    end: { hour: 24, minute: 0 },
  });
  add(open, WeekDay.Thursday, {
    start: { hour: 0, minute: 0 },
    end: { hour: 24, minute: 0 },
  });
  add(open, WeekDay.Friday, {
    start: { hour: 0, minute: 0 },
...
export const buildStocksWeeklySchedule = (
  currentDate: Date = new Date()
): Schedule => {
  const open = emptyWeekly();
  const lowLiq = emptyWeekly();
  ...
  const summary =
    "Monday - Friday: 9:35 am - 4:00 pm ET (Closed weekends & holidays)";
  return { open, lowLiq, holidays, summary };
};

export const buildIndicesWeeklySchedule = (
  currentDate: Date = new Date()
): Schedule => buildStocksWeeklySchedule(currentDate);

export const buildCommoditiesWeeklySchedule = (
  currentDate: Date = new Date()
): Schedule => {
  const open = emptyWeekly();
  const lowLiq = emptyWeekly();
