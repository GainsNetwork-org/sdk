...
export const fetchPairs = async (contracts: Contracts, pairIxs: PairIndex[]): Promise<Pair[]> => {
  if (!contracts) {
    return [];
  }
...
export const fetchFees = async (contracts: Contracts, feeIxs: PairIndex[]): Promise<Fee[]> => {
  if (!contracts) {
    return [];
  }
