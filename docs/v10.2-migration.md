...
const pairDepthBands = await diamond["getPairDepthBands(uint256[])"](pairIndices);
...
const decodedMultiple = await diamond["getPairDepthBandsDecoded(uint256[])"](pairIndices);
