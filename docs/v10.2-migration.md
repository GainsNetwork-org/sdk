...
NOTE: If you're using the builder pattern, this is handled for you.
...
// After - Fetching depth bands
const singlePairBands = await diamond.getPairDepthBands(pairIndex);

// Multiple pairs
const pairDepthBands = await diamond.getPairDepthBandsArray(pairIndices);
...
// Multiple pairs
const decodedMultiple = await diamond.getPairDepthBandsDecodedArray(pairIndices);
